...
public class ReflectionHelper
{
	...
	/**
	 * Obtain a class by name using the specified class loader.
	 * Wrapper around {@link net.minecraftforge.fml.relauncher.ReflectionHelper#getClass(ClassLoader, String...)}, included for convenience/competeness.
	 */
	public static Class<? super Object> getClass(ClassLoader loader, String... classNames)
	{
		return net.minecraftforge.fml.relauncher.ReflectionHelper.getClass(loader, classNames);
	}
	...
	/**
	 * Obtain an array of all the fields of a given type from class declaringClass.
	 */
	public static final <T> Field[] getDeclaredFields(final Class<T> type, final Class<?> declaringClass, boolean isStatic, boolean isFinal)
	{
		final List<Field> result = new ArrayList<Field>();
		for (Field field : declaringClass.getDeclaredFields())
		{
			final int modifiers = field.getModifiers();
			if (field.getType().isAssignableFrom(type) && (isStatic == Modifier.isStatic(modifiers)) && (isFinal == Modifier.isFinal(modifiers)))
			{
				result.add(field);
			}
		}
...
//	}
	...
	public static final Method getMethod(final Class<?> declaringClass, String methodName, Class<?>... parameterTypes)
	{
	...
	/**
	 * Obtain the values from an array of static Fields fieldsToAccess of type fieldType.
	 */
	public static final <T, E> T[] getFieldValues(final Field[] fieldsToAccess, final Class<T> fieldType)
	{
		final List<T> result = new ArrayList<T>();
		for (Field field : fieldsToAccess)
		{
			final int modifiers = field.getModifiers();
			if (field.getType().isAssignableFrom(fieldType) && Modifier.isStatic(modifiers))
			{
				try
				{
					result.add((T)field.get(null));
		...
		return result.toArray((T[])Array.newInstance(fieldType, result.size()));
	}

	/**
	 * Obtain the values from an array of Fields fieldsToAccess of type fieldType from an Object instance.
	 */
	public static final <T, E> T[] getFieldValues(final Field[] fieldsToAccess, E instance, final Class<T> fieldType)
	{
		final List<T> result = new ArrayList<T>();
		for (Field field : fieldsToAccess)
		{
			if (field.getType().isAssignableFrom(fieldType))
			{
				try
				{
					result.add((T)field.get(instance));
				}
				catch (Exception ex)
				{
					throw new UnableToAccessFieldException(new String[0], ex);
	...
	public static final <T> void setFinalStatic(final Field fieldToAccess, T value)
	{
		try
		{
