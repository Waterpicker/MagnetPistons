...
public class BlockPistonBase extends net.minecraft.block.BlockPistonBase
{
	...
	public boolean eventReceived(IBlockState state, World world, BlockPos pos, int id, int param)
	{
		...
		else if (id == 1)
		{
			...
			world.setBlockState(pos, ModBlocks.PISTON_EXTENSION.getDefaultState().withProperty(BlockPistonMoving.FACING, enumfacing).withProperty(BlockPistonMoving.TYPE, this.isSticky ? EnumPistonType.STICKY : EnumPistonType.DEFAULT), 3);
	...
	private boolean doMove(World world, BlockPos pos, EnumFacing direction, boolean extending)
	{
		...
		else
		{
			...
			for (int l = list.size() - 1; l >= 0; --l)
			{
				...
				world.setBlockState(blockpos3, ModBlocks.PISTON_EXTENSION.getDefaultState().withProperty(FACING, direction), 4);
			...
			if (extending)
			{
				...
				IBlockState iblockstate1 = ModBlocks.PISTON_EXTENSION.getDefaultState().withProperty(BlockPistonMoving.FACING, direction).withProperty(BlockPistonMoving.TYPE, this.isSticky ? EnumPistonType.STICKY : EnumPistonType.DEFAULT);
			...
			if (extending)
			{
				world.notifyNeighborsOfStateChange(blockpos2, ModBlocks.PISTON_HEAD, false);
			}
